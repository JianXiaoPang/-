...
- (void)initMessage {
    if (_message != nil) {
        ...
        _messageLabel.textColor = RGBA(102, 102, 102, 1.0);
...
- (void)initAllButtons {
    if (_buttonTitleArray.count > 0) {
        ...
        for (int i=0;i<_buttonTitleArray.count;i++){
            NSString *buttonTitle =[_buttonTitleArray objectAtIndex:i];
            ...
            [button setTitleColor:RGBA(251, 162, 0, 1.0) forState:UIControlStateNormal];
            [button addTarget:self action:@selector(buttonWithPressed:) forControlEvents:UIControlEventTouchUpInside];
            if(i==1){
                [button setBackgroundColor:RGBA(235, 235, 235, 1.0)];
            }
