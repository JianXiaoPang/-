...
- (void)viewDidLoad {
    ...
    self.contview.frame = CGRectMake(0, 0, kDeviceWidth, 64);
    self.btn1.frame = CGRectMake(kDeviceWidth-40, 22, 40, 40);
    self.btn2.frame = CGRectMake(kDeviceWidth-70, 22, 40, 40);
    self.URLArray = [[NSMutableArray alloc]init];
    _myWebView = [[UIWebView alloc]init];
    _myWebView.delegate =self;
    _myWebView.frame = CGRectMake(0, 65, kDeviceWidth, KDeviceHeight-65);
...
- (IBAction)goBack:(UIButton *)sender {
    ...
    else
    {
        ...
        [_myWebView reload];
        NSString *URLStr = [NSString stringWithFormat:@"%@",[self.URLArray lastObject]];
        ...
        if(jsonStr.length>0){
            ...
            if(!error){
                ...
                if(JSkey == 1){
                    ...
                    if (reDic.count > 0) {
                        shareModel.returnDic = dic[@"Report"];
                    }
                    ...
                    //调用浏览器
                    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",dic[@"Data"][@"url"]]]];
                }
            }
        }
    }
